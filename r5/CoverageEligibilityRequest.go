...
func (resource *CoverageEligibilityRequest) T_EventType(numEvent int, optionsValueSet []Coding, htmlAttrs string) templ.Component {
	if resource == nil || numEvent >= len(resource.Event) {
		return CodeableConceptSelect("CoverageEligibilityRequest.Event["+strconv.Itoa(numEvent)+"].Type", nil, optionsValueSet, htmlAttrs)
	}
	return CodeableConceptSelect("CoverageEligibilityRequest.Event["+strconv.Itoa(numEvent)+"].Type", &resource.Event[numEvent].Type, optionsValueSet, htmlAttrs)
}
func (resource *CoverageEligibilityRequest) T_EventWhenDateTime(numEvent int, htmlAttrs string) templ.Component {
	if resource == nil || numEvent >= len(resource.Event) {
		return DateTimeInput("CoverageEligibilityRequest.Event["+strconv.Itoa(numEvent)+"].WhenDateTime", nil, htmlAttrs)
	}
	return DateTimeInput("CoverageEligibilityRequest.Event["+strconv.Itoa(numEvent)+"].WhenDateTime", &resource.Event[numEvent].WhenDateTime, htmlAttrs)
}
