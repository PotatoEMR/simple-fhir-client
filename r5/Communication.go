...
func (resource *Communication) T_PayloadContentCodeableConcept(numPayload int, optionsValueSet []Coding, htmlAttrs string) templ.Component {

	if resource == nil || numPayload >= len(resource.Payload) {
		return CodeableConceptSelect("Communication.Payload."+strconv.Itoa(numPayload)+"..ContentCodeableConcept", nil, optionsValueSet, htmlAttrs)
	}
	return CodeableConceptSelect("Communication.Payload."+strconv.Itoa(numPayload)+"..ContentCodeableConcept", &resource.Payload[numPayload].ContentCodeableConcept, optionsValueSet, htmlAttrs)
}
