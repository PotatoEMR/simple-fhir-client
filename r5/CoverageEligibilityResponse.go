...
func (resource *CoverageEligibilityResponse) T_EventId(numEvent int) templ.Component {

	if resource == nil || len(resource.Event) >= numEvent {
		return StringInput("CoverageEligibilityResponse.Event["+strconv.Itoa(numEvent)+"].Id", nil)
	}
	return StringInput("CoverageEligibilityResponse.Event["+strconv.Itoa(numEvent)+"].Id", resource.Event[numEvent].Id)
}
func (resource *CoverageEligibilityResponse) T_EventType(numEvent int, optionsValueSet []Coding) templ.Component {

	if resource == nil || len(resource.Event) >= numEvent {
		return CodeableConceptSelect("CoverageEligibilityResponse.Event["+strconv.Itoa(numEvent)+"].Type", nil, optionsValueSet)
	}
	return CodeableConceptSelect("CoverageEligibilityResponse.Event["+strconv.Itoa(numEvent)+"].Type", &resource.Event[numEvent].Type, optionsValueSet)
}
...
func (resource *CoverageEligibilityResponse) T_ErrorExpression(numError int, numExpression int) templ.Component {

	if resource == nil || len(resource.Error) >= numError || len(resource.Error[numError].Expression) >= numExpression {
		return StringInput("CoverageEligibilityResponse.Error["+strconv.Itoa(numError)+"].Expression["+strconv.Itoa(numExpression)+"]", nil)
	}
	return StringInput("CoverageEligibilityResponse.Error["+strconv.Itoa(numError)+"].Expression["+strconv.Itoa(numExpression)+"]", &resource.Error[numError].Expression[numExpression])
}
