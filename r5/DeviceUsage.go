...
func (resource *DeviceUsage) T_AdherenceReason(numReason int, optionsValueSet []Coding, htmlAttrs string) templ.Component {
	if resource == nil || numReason >= len(resource.Adherence.Reason) {
		return CodeableConceptSelect("AdherenceReason["+strconv.Itoa(numReason)+"]", nil, optionsValueSet, htmlAttrs)
	}
	return CodeableConceptSelect("AdherenceReason["+strconv.Itoa(numReason)+"]", &resource.Adherence.Reason[numReason], optionsValueSet, htmlAttrs)
}
