...
func (resource *ClinicalUseDefinition) T_Library(numLibrary int) templ.Component {

	if resource == nil || len(resource.Library) >= numLibrary {
		return StringInput("ClinicalUseDefinition.Library["+strconv.Itoa(numLibrary)+"]", nil)
	}
	return StringInput("ClinicalUseDefinition.Library["+strconv.Itoa(numLibrary)+"]", &resource.Library[numLibrary])
}
