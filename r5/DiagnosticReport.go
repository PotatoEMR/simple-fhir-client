...
// json -> struct, first reject if resourceType != DiagnosticReport
func (r *DiagnosticReport) UnmarshalJSON(data []byte) error {
	if err := json.Unmarshal(data, &checkType); err != nil {
		return err
	} else if checkType.ResourceType != "DiagnosticReport" {
		return errors.New("resourceType not DiagnosticReport")
	}
	return json.Unmarshal(data, (*OtherDiagnosticReport)(r))
}
...
func (resource *DiagnosticReport) T_Study(frs []FhirResource, numStudy int, htmlAttrs templ.Attributes) templ.Component {
...
func (resource *DiagnosticReport) T_Composition(frs []FhirResource, htmlAttrs templ.Attributes) templ.Component {
	if resource == nil {
		return ReferenceInput(frs, "composition", nil, htmlAttrs)
	}
	return ReferenceInput(frs, "composition", resource.Composition, htmlAttrs)
}
...
func (resource *DiagnosticReport) T_SupportingInfoReference(frs []FhirResource, numSupportingInfo int, htmlAttrs templ.Attributes) templ.Component {
	if resource == nil || numSupportingInfo >= len(resource.SupportingInfo) {
		return ReferenceInput(frs, "supportingInfo["+strconv.Itoa(numSupportingInfo)+"].reference", nil, htmlAttrs)
	}
	return ReferenceInput(frs, "supportingInfo["+strconv.Itoa(numSupportingInfo)+"].reference", &resource.SupportingInfo[numSupportingInfo].Reference, htmlAttrs)
}
