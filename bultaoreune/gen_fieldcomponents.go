...
`
	//different versions have different types -> need different form inputs for those types or cant have form inputs for types they dont have
	//seems like biggest different is r4 -> r4b they introduced codeableReference
	if fhirVersion == "r4" {
		ret = ret + substanceAmount + prodCharacteristic + population
	} else if fhirVersion == "r4b" {
		ret = ret + codeableReference + ratioRange + prodCharacteristic + population
	} else if fhirVersion == "r5" {
		ret = ret + codeableReference + ratioRange + extendedContactDetail + availability + monetaryComponent + virtualServiceDetail
	}
	return ret
}

var codeableReference = `
templ CodeableReferenceInput(fieldname string, current *CodeableReference, attrs templ.Attributes) {
	<p>CodeableReference not done</p>
}`
var ratioRange = `
templ RatioRangeInput(fieldname string, current *RatioRange, attrs templ.Attributes) {
	<p>RatioRange not done</p>
}`

// var relativeTime = `
//
//	templ RelativeTimeInput(fieldname string, current *RelativeTime, attrs templ.Attributes) {
//		<p>RelativeTime not done</p>
//	}`
var extendedContactDetail = `
templ ExtendedContactDetailInput(fieldname string, current *ExtendedContactDetail, attrs templ.Attributes) {
	<p>ExtendedContactDetail not done</p>
}`
var availability = `
templ AvailabilityInput(fieldname string, current *Availability, attrs templ.Attributes) {
	<p>Availability not done</p>
}`
var monetaryComponent = `
templ MonetaryComponentInput(fieldname string, current *MonetaryComponent, attrs templ.Attributes) {
	<p>MonetaryComponent not done</p>
}`
var virtualServiceDetail = `
templ VirtualServiceDetailInput(fieldname string, current *VirtualServiceDetail, attrs templ.Attributes) {
	<p>VirtualServiceDetail not done</p>
}`
var substanceAmount = `
templ SubstanceAmountInput(fieldname string, current *SubstanceAmount, attrs templ.Attributes) {
	<p>SubstanceAmount not done</p>
}`
var prodCharacteristic = `
