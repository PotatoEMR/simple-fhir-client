...
func main() {
	...
	for _, fhirVersion := range fhirVersions {
		...
		//write structs
		fileToStructs(path.Join(extractDirVer, "profiles-types.json"), generateDirVer, fhirVersion)
		//write resources
		fileToStructs(path.Join(extractDirVer, "profiles-resources.json"), generateDirVer, fhirVersion)
		fmt.Println("created resources in", generateDirVer)
		fileToClient(path.Join(extractDirVer, "profiles-resources.json"), generateDirVer, fhirVersion, generateClientDirVer)
...
// endregion
func fileToStructs(spec_file, generateStructsDir, fhirVersion string) {
	...
	for _, entry := range bundle.Entry {
		...
		today := time.Now()
		fmt.Fprintf(file, "//generated %s %d %d with command go run ./bultaoreune %s\n", today.Month(), today.Day(), today.Year(), strings.Join(os.Args[1:], " "))
		fmt.Fprintf(file, "//inputs %s/[%s]\n", fhirURL+"/"+fhirVersion, strings.Join(zipFileNames, " "))
		fmt.Fprintf(file, "//for details see https://github.com/PotatoEMR/simple-fhir-client\n\n")
		fmt.Fprintf(file, "package %s\n\n", fhirVersion)

		if isDomainResource {
			fmt.Fprintf(file, "import \"encoding/json\"\n")
			//fmt.Fprintf(file, "import \"net/url\"\n") //will need for encode search params
		}
		if slices.Contains(resImports, res.Name) {
			fmt.Fprintf(file, "import \"strings\"\n")
		}
		if res.Name == "Bundle" {
		...
		for _, s_name := range structInsertionOrder {
			...
			for _, elt := range s_elts {
				...
				for _, t := range elt.Type {
					...
					fmt.Fprintf(file, "%s %s%s `json:\"%s%s\"`\n", f, c, ft, fl, omitempty)
				}
			}
...
// region Client
func fileToClient(spec_file, generateDirVer, fhirVersion, generateClientDirVer string) {
