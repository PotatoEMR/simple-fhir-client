...
func (resource *Task) T_ReasonReference(frs []FhirResource, htmlAttrs templ.Attributes) templ.Component {
	if resource == nil {
		return ReferenceInput(frs, "reasonReference", nil, htmlAttrs)
	}
	return ReferenceInput(frs, "reasonReference", resource.ReasonReference, htmlAttrs)
}
...
func (resource *Task) T_InputValueContributor(numInput int, htmlAttrs templ.Attributes) templ.Component {
	if resource == nil || numInput >= len(resource.Input) {
		return ContributorInput("input["+strconv.Itoa(numInput)+"].valueContributor", nil, htmlAttrs)
	}
	return ContributorInput("input["+strconv.Itoa(numInput)+"].valueContributor", &resource.Input[numInput].ValueContributor, htmlAttrs)
}
...
func (resource *Task) T_OutputValueContributor(numOutput int, htmlAttrs templ.Attributes) templ.Component {
	if resource == nil || numOutput >= len(resource.Output) {
		return ContributorInput("output["+strconv.Itoa(numOutput)+"].valueContributor", nil, htmlAttrs)
	}
	return ContributorInput("output["+strconv.Itoa(numOutput)+"].valueContributor", &resource.Output[numOutput].ValueContributor, htmlAttrs)
}
