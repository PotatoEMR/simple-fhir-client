...
// json -> struct, first reject if resourceType != ChargeItem
func (r *ChargeItem) UnmarshalJSON(data []byte) error {
	if err := json.Unmarshal(data, &checkType); err != nil {
		return err
	} else if checkType.ResourceType != "ChargeItem" {
		return errors.New("resourceType not ChargeItem")
	}
	return json.Unmarshal(data, (*OtherChargeItem)(r))
}
...
func (resource *ChargeItem) T_Context(frs []FhirResource, htmlAttrs templ.Attributes) templ.Component {
	if resource == nil {
...
func (resource *ChargeItem) T_PriceOverride(htmlAttrs templ.Attributes) templ.Component {
	if resource == nil {
		return MoneyInput("priceOverride", nil, htmlAttrs)
	}
	return MoneyInput("priceOverride", resource.PriceOverride, htmlAttrs)
}
...
func (resource *ChargeItem) T_Service(frs []FhirResource, numService int, htmlAttrs templ.Attributes) templ.Component {
	if resource == nil || numService >= len(resource.Service) {
		return ReferenceInput(frs, "service["+strconv.Itoa(numService)+"]", nil, htmlAttrs)
	}
	return ReferenceInput(frs, "service["+strconv.Itoa(numService)+"]", &resource.Service[numService], htmlAttrs)
}
func (resource *ChargeItem) T_ProductReference(frs []FhirResource, htmlAttrs templ.Attributes) templ.Component {
	if resource == nil {
