...
func (resource *Goal) T_OutcomeCode(numOutcomeCode int, optionsValueSet []Coding) templ.Component {

	if resource == nil || len(resource.OutcomeCode) >= numOutcomeCode {
		return CodeableConceptSelect("Goal.OutcomeCode["+strconv.Itoa(numOutcomeCode)+"]", nil, optionsValueSet)
	}
	return CodeableConceptSelect("Goal.OutcomeCode["+strconv.Itoa(numOutcomeCode)+"]", &resource.OutcomeCode[numOutcomeCode], optionsValueSet)
}
