...
func (resource *AuditEvent) T_SourceSite(htmlAttrs string) templ.Component {
	if resource == nil {
		return StringInput("SourceSite", nil, htmlAttrs)
	}
	return StringInput("SourceSite", resource.Source.Site, htmlAttrs)
}
func (resource *AuditEvent) T_SourceType(numType int, optionsValueSet []Coding, htmlAttrs string) templ.Component {
	if resource == nil || numType >= len(resource.Source.Type) {
		return CodingSelect("SourceType["+strconv.Itoa(numType)+"]", nil, optionsValueSet, htmlAttrs)
	}
	return CodingSelect("SourceType["+strconv.Itoa(numType)+"]", &resource.Source.Type[numType], optionsValueSet, htmlAttrs)
}
