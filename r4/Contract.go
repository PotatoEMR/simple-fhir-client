...
func (resource *Contract) T_TermActionReasonCode(numTerm int, numAction int, numReasonCode int, optionsValueSet []Coding, htmlAttrs templ.Attributes) templ.Component {
...
func (resource *Contract) T_TermActionReasonReference(frs []FhirResource, numTerm int, numAction int, numReasonReference int, htmlAttrs templ.Attributes) templ.Component {
	if resource == nil || numTerm >= len(resource.Term) || numAction >= len(resource.Term[numTerm].Action) || numReasonReference >= len(resource.Term[numTerm].Action[numAction].ReasonReference) {
		return ReferenceInput(frs, "term["+strconv.Itoa(numTerm)+"].action["+strconv.Itoa(numAction)+"].reasonReference["+strconv.Itoa(numReasonReference)+"]", nil, htmlAttrs)
	}
	return ReferenceInput(frs, "term["+strconv.Itoa(numTerm)+"].action["+strconv.Itoa(numAction)+"].reasonReference["+strconv.Itoa(numReasonReference)+"]", &resource.Term[numTerm].Action[numAction].ReasonReference[numReasonReference], htmlAttrs)
}
